...
END;";
        ...
        public void LogFail(string migrationName)
        {
            var sql = $"MERGE INTO Migration_history USING dual ON (migration='{migrationName}') " +
                ...
                $"WHEN NOT MATCHED THEN INSERT VALUES ('{migrationName}', :current_date, :current_date, 0)";

            using var command = new OracleCommand(sql, connection);
            command.Parameters.Add("current_date", OracleDbType.TimeStamp, DateTime.Now, System.Data.ParameterDirection.Input);
            command.ExecuteNonQuery();
        }
